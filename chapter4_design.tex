...
atop a large, nearly vertical display screen. The sensor starts tracking
points when an object is sufficiently close to the display screen. The fact that
touch points can be detected in mid-air has several implications. One, it is
quite possible to create false positives due to hand orientation and the
gestures being performed, we discuss our heuristics to counter balance these
artifact in the implementation chapter. Two, gestures done with single hand,
multi-fingers tend to be unreliable, the sensor can lose track of the touch
points or introduce ambiguities through false-positive points. For example,
picture a one handed pinching gesture with the thumb and index fingers, we found
that often times additional tracking points are formed inadvertently by the
knuckles of the pinky or ring fingers. Because there are significant distance
among these points, it is difficult to judge which points are real without
additional information. Since we are only provided with XY coordinates
information, we decided to design our interactions around single finger
gestures. While we lose the opportunity to create unique gestures, we gain in
return more reliable system.
