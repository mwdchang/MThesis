...
similar style as the scented widget approach in \cite{Willett2007}. The
histogram bars are shaded in light grey, except for the selected item which
we render in blue for distinction. The additional histogram makes it very clear
any outlier items, and allows reader to make relative comparison among items.
Toggling the widget will activate an animated transition to open/close the item
list, which can be scrolled and selected. Selected item is shown in bold font.
...
We intend to have our application running on a walk-up-and-use scenario
on large displays, thus we have built our prototype to take into account
keyboard/mouse free interactions.
We use the idea of ``zones'' to segment our display space and to process touch
point events. Each interactive element has an implicit zone that is calculated
when a touch point is detected by the sensor. An overview of different zones are as follows:
\begin{itemize}[noitemsep]
  \item Visualization Zone: The main \threed visualization
  \item Time Block Zone: Covers the year and month time sliders 
  \item Filter Zone: Covers the accessible hierarchy filters
  \item Lens Zone: Area covered by the lens widget
...
the visualization zone. This creates a continuous interaction and avoid
distracting the users by sudden transition of different zones.
...
movements instead of real touches, as such it is quite possible to introduce
false positives due to how the hand is oriented. While it is possible to use
heuristics to mitigate the consequences of unintended noises, there are
ambiguous cases where we cannot guarantee the correct outcomes. For  example,
imagine a single handed pinch gesture with the thumb and index  finger, we have
noticed through experimental trials that the knuckles on the  other fingers are
sometimes picked up as touch points as well. This case  creates a problem,
because the points are roughly the same distance apart, it is difficult to judge
which points are actually created by the thumb and index finger without
additional information. Because we only receive normalized coordinates from the
sensor device, we opted to abandon any complex, single-handed, multi-finger
gestures in order to have a system will be able to react inline with respect
to user expectations.

We design two types of touch gesture semantics for our system: a tap semantic
and a hold semantic. The tap semantic results from a single touch where there is
no delay between the initial touch gesture and subsequent actions, conversely
the hold semantic requires the user to hold the initial touch point briefly
before transitioning into subsequent movements. We map the tap semantic to
actions to selection, scrolling and orientation logic. For the gestures that use
the hold semantics, we reserve these gestures for the toggling our custum made
