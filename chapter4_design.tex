...
Activate Document & Empty & Space & 2-Finger Tap \\
...
is the aggregated score, it reflects the overall importance of the entity with 
relations to other entities. The difference is the score of entity one minus the 
score of entity two, the sign component, and the magnitude component of the
difference reflects which side has the stronger influence. To illustrate with 
an example: if we calculated the wheel entity score to be 15 on vehicle A and 20
on vehicle B, the sum would then be 35 and the difference score would be -5.

We encode the sum score as the colour of the \threed component, based on the
default colour scale. For the difference, we render the measure as an outline around the object. The sign of the 
