...
and resizing the lens, we can identify the front, or any other logical region quite
easily.
...
layout is eye-pleasing, but is unstable due to a single anchor point in the centre, 
and it is harder to compare across heatmaps because they re not spatially aligned, 
thus it was deemed unsuitable for our system.
...
We set the maximum visible heatmap to 8 in this prototype.
...
%create visual clutter, we employed a scrolling mechanism that 
...
used for relocating the document widget to a different position. The right
region is designated for scrolling over the document texts.
...
people to focus and filter based on specific seasons.

Interactions with the sliders are done through their markers as mentioned above, or directly
on the histogram bars. A double select action on the year slider's bars provides a short cut
to select an entire year. An animated transition is used to interpolate the histograms to its
new state.
...
point events. Each zone consists of one or more polygonal spaces with predefined
...
false positives due to hand posture and orientation. We can use software
...
conjecture a hand orientation. As we only have XY coordinates as our data source, 
we cannot guarantee correctness.  Thus, we decided
to abandon any complex, explicitly-singled-handed, multi-fingered gestures in
favour of a simpler, less error prone approach.

The final gesture set is a cumulation of several iterations of redesign. At 
each iteration, fellow lab members were asked to pilot-test the new gesture
...
where the initial position is held for less than a threshold, while a long-touch is the
opposite. The system uses 450 milliseconds as the threshold. From the above, we create
more complex gesture set:
...
\begin{itemize} [noitemsep]
  ...
  Both zones uses the drag gesture. Both  reposition and scroll actions use the drag gesture.

  \item Hierarchical Filters: A single touch gesture is used to open, close and to
  make selections. Scrolling of the item list is achieved by dragging the item list vertically.
...
physical response, we implemented out own visual feedback. We created
...
\begin{itemize}[noitemsep]
  ...
  time elapsed. A long touch gesture is activated once the arc has travelled
  ...
  smaller circles, similar to that of short touch points.
