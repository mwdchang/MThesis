...
the accuracy of its semantic interpretation. WordsEye also works best for
...
This is in contrast with photorealistic rendering where the goal is to compared
the rendering against a ground-truth image. Second, often times it is
undesirable to preserve a high degree of accuracy, specular reflections, shadows and other natural
...
done in order to differentiate normal and outlier regions in the final
...
thesis is perhaps mostly closely associated with CarComVis, however our
renderings are driven by unstructured text, and our interactions are directly
applicable to the visualization itself rather than through multiple coordinated
views.
...
Fisheye Lens~\cite{Sarkar1992} is a well known technique for enlarging
...
focus is, because the lens itself may occludes nearby objects which the viewers
...
NPRLens supports many of the effects discussed in the NPR section above.
...
although the viewers cannot see into the cursor itself, the cursor creates a
...
like behaviour, that is, the the systems queries the objects that are in
proximity to the path drawn. Additional information about these objects are then
