...
Descriptive rendering section reviews several text-to-scene applications.
A discussion of why NPR is beneficial is next, followed by recent examples of
visualizations that combines abstract and concrete concepts. Next, we review
...
the users change parameters to explore different parts of the visualization.
...
is advocated in Shneiderman's information seeking mantra~\cite{Shneiderman1996}
and can be broken down into these specific tasks:
\begin{itemize}[noitemsep]
...
Wordle~\cite{VIE2009a}. Unlike previous systems mentioned above, Wordle does
...
Another major benefits of NPR is that it can be used to simplify orr
...
features are view dependent in both \threed and \twod scenarios, this usually
indicates that feature extraction operation need to run once per frame.
...
reconstructions~\cite{Masuch1998, Strothotte1999}, found that straight, solid
strokes convey higher degree of certainty than strokes that are drawn faded and
curved. The relevance here to our work is that we use this type of technique to
covey objects that are not in the focused context.
...
techniques look at visibility issues, to ensure that semantically important
objects are easily visible and to provide features to deal with occlusion
issues. One prominent research came from an importance-driven function that can
be used to determine the visibility of objects \cite{Viola2004}. In this paper
scores are assigned to volumetric components such that more important
...
context categories, with the following semantics:
\begin{itemize}[noitemsep]
...
InfoVis. Our work here is perhaps mostly closely associated with CarComVis,
however our renderings are driven by unstructured text, and our interactions are
directly applicable to the visualization itself rather than through coordinated
views.
...
graphical constructs such as line segment and curve primitives. These can then
be further manipulated such as stretching, shrinking or changing the stroke styles of each
primitive unit. NPRLens supports many of the effects discussed in the NPR
section above.
...
possible to use buffer the rendering into textures and combine them at a later
stage, rather than physically cutting the scene in volumes, this is the
technique that we use here.
...
interesting regions. Extended Excentric Labeling~\cite{Bertini2009} further extends on the idea by
...
users to draw a query path onto a segmented volume data. The path behaves as a visual query, that is, a query
...
path are highlighted, and their semantic information are displayed in a user
interface panel.
...
leveraging public opinion to come to a consensus. Wobbrock \etal conducted
experiments where the consequences are given, and participants had to come up
with their own appropriate gestures to cause the said
...
attributes in gesture design that one needs to pay attention to to make them
