...
preattentive processing \cite{WAR2004b}. In theory, this perceptual systeme
...
cel-shading/toon-shading techniques (???). Generally speaking, cel-shading and
...
mouse or keyboard. Much of the challenges today seem to focus on two central
themes: collaboration and gesture design. Collaboration deals with the idea of
having personal and collaborative spaces, as well how people interact with each
other either in a co-located, or distributed setting. Gestures, on the other
hand, deals with classification and design of gestures that are natural and
intuitive to use. In this work, we focus on the gesture design aspect.

As a general reference, Wigdor and Wixon in their book touched upon the idea of
natural user interfaces (NUI), where they emphasis that NUIs are meant to be
intuitive to use, rather than a strictly organic process \cite{Wigdor2011}.
Additionally they look at interactive surface computing from a historical
perspective, with many DOs and DO NOTs and design implications.

Other researchers looked at gesture design using a crowd-sourcing approach.
Wobbrock \etal conducted an experiment where the consequences are given, and the
participants needed to come up with an appropriate gesture to match the end
result \cite{Wobbrock2009}. They are able to come up with a classification of
generally used gesture set. Others, such as Hinrich and Carpendale, looked
at how environment and social context affect how gestures are performed
\cite{Hinrich2011}. They found that in general gestures are not executed in
isolation, but dependent on previous gestures, as well as the social context and
demographic of the executor.

While in this project our goal is not centred around the creation of a new set
of gestures, the findings and experimental approaches were inspirations to how
we designed our interaction techniques.
