...
comparisons, note these scenarios are contrived and not based on real data. The
third and last scenario, we take a look at a real world event and see if the 
dataset reveal any interesting facts surrounding the event.

 
\subsection{Locating Issues}
This scenario describes how a regular consumer, Larry, may use the 
visualization to look up a problem. Larry has about 6 years of driving 
experience and owns a 2005 Ford Focus, recently while driving he 
noticed a rattling sound coming from the passenger side of his vehicle. 
He decided to do some research on his own before taking the car back 
to the dealership.

Using the visualization, Larry filters the dataset to focus on 2005 
Focus model. Since Larry is not sure exactly where the noise came from, 
...
in the focused region. Larry then selects the suspension component using 
...
there are at least 4 or 5 reports that seemed to document similar noise 
...
Sara sees that engine and wheel components are lightly shaded, which indicates
that neither one had serious issues regarding these sub systems. However, she notices
that there are dark green outlines around several components, using the lens she
identifies them as parts of the transmission subsystem. It seems like Sentra had
a much higher rate of transmission failure than Civic. Sara decides that her original
intuition about Honda Civic is correct.
...
for example the manufacturer``Toyota Motors'' was replaced with ``MFR1.''
...
had difficulties understanding the co-occurrence concept. Unexpectedly, 2 2
...
%We also observed different behaviours, some
...
\begin{itemize}[noitemsep]
  \item Non consecutive year functions: Allow non-contiguous selection, for
...
requires multiple steps, result tend to vary. In the open-ended tasks, we
