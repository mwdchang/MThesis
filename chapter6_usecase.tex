...
This scenario describes how a regular consumer, Jason, may use the 
visualization to research a problem. Jason has about three years of driving 
...
Using the visualization, Jason filters the dataset to focus on his
vehicle model. Since Jason is not sure exactly where the noise came from, 
he uses the lens widget to focus on components near the front-passenger 
region. Using the lens, Jason can see that the suspension component has a
higher number of complaints registered against it than the other components 
in the focused area. Jason then selects the suspension component using 
the heatmap, and toggles the document widget so he can read through the 
actual complaint reports. After a few minutes of reading, Jason notes that
there are at least eight or nine reports that seemed to document similar noise 
issues and point to defective suspension setup. Jason decides that he 
...
% on each widget can be see in Figure \ref{chart:usage}.
	...
	\begin{figure}
	 ...
	 \caption[Widget Usage Visualization.]{Widget usage visualization for the task
	 of comparing two vehicles.}
