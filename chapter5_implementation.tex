...
\begin{itemize}[noitemsep]
  ...
  The basic idea is that State Manager represents a save-state, that can be
  written out to disk and restored.
  ...
  that impact the occurrence and co-occurrence scores. On system initialization,
  Cache Manager reads in all entity scores, and compute what is needed on a
  ad-hoc basis.
...
changes. There are also pathological cases where polygons intersects each other,
...
order-dependent terms, but there is a threshold on the amount
of transparency that can be applied on an object~\cite{Meshkin2007, Bavoil2008}.
...
accurate results, albeit bounded by hardware constraints and the complexity of
the scene itself.  
...
greatly, database query performance tend to vary as well. To compound the
...
false-positive touch points as a result of drifting too close to the screen
itself.
...
result from minute movements. While these are tuned specifically for our
hardware, we believe rules are general enough that they can be adopted to other
touch sensors. 
...
\begin{itemize} [noitemsep]
  ...
  points that are in transition. New touch points cannot be created in the
  zones, existing touch points are not affected.
